...
def get_output_channels(onnx_nodes, initializers, node_name):
    """
    Helper function to get the number of output channels from the previous node.
    """
    # For simplicity, assume output channels equal input channels for certain ops
    for node in onnx_nodes:
        if node.name == node_name:
            if node.op_type == 'Conv':
                # Get the weight tensor associated with this Conv node
                weight_tensor_name = node.input[1]
                for init in initializers:
                    if init.name == weight_tensor_name:
                        # The first dimension is the number of output channels
                        return init.dims[1]
            elif node.op_type in ['Relu', 'MaxPool', 'Transpose']:
                # These ops do not change the number of channels
                # Need to recurse to find the input channels
                return get_output_channels(onnx_nodes, initializers, node.input[0])
            else:
                # Default to 1 channel if unable to determine
                return 1
    # Default to 1 channel if node not found
    return 1

def convert_attribute_to_type(attr_value, attr_type):
    supported_types = {
    ...
    for node, attrs in nodes:
        ...
        for att, (att_type, val) in attrs['features']['attributes'].items():
            if ast.literal_eval(val) is not None:
                node_attrs[att] = convert_attribute_to_type(val, att_type)
